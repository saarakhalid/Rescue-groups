...
const senssyData = {
    ...
    height: "42cm",
    weight: "5.4kg",
    description: "With a coat as soft as silk and eyes that sparkle with curiosity, Senssy is a charming cat looking for a family to call his own. His gentle purrs will surely melt your heart. Consider giving Senssy a loving home filled with treats and cuddles!",
};

document.addEventListener('DOMContentLoaded', function () {
    // Update the page content with Senssy's information
    document.querySelector('.title').textContent = `I Enjoy Treats and Cuddling - Meet ${senssyData.name}`;
    document.querySelector('img').alt = `${senssyData.name} the Cat`;
    document.querySelector('.location').textContent = `Location: ${senssyData.location}`;

    // Update buttons
    document.querySelectorAll('.buttons button')[0].textContent = `Age: ${senssyData.age}`;
    document.querySelectorAll('.buttons button')[1].textContent = `Sex: ${senssyData.sex}`;
    document.querySelectorAll('.buttons button')[2].textContent = `Height: ${senssyData.height}`;
    document.querySelectorAll('.buttons button')[3].textContent = `Weight: ${senssyData.weight}`;
...
function adoptSenssy() {
    ...
    simulateAdoptionProcess()
        .then(() => {
            // Adoption successful
            alert('Adoption request for Senssy sent successfully! Thank you for your interest.');
        })
        .catch((error) => {
            // Adoption failed
            console.error('Error in adoption process:', error);
            alert('Sorry, there was an error processing your adoption request. Please try again later.');
        });
}
